...
public class CrptApi {
    ...
    public CrptApi(TimeUnit timeUnit, int requestLimit) {
        this.requestLimit = requestLimit > 0 ? requestLimit : 5;
        this.httpClient = HttpClient.newHttpClient();
        this.objectMapper = new ObjectMapper();
        this.semaphore = new Semaphore(requestLimit);
        long timeInterval = timeUnit.toMillis(1);
        scheduleResetRequestCount(timeInterval);
    }


    private void scheduleResetRequestCount(long timeInterval) {
        Thread scheduler = new Thread(() -> {
            while (true) {
                try {
                    ...
                    System.out.println("Ошибка планировщика");
                }
            }
        });
    ...
    public void createDocument(Document document, String signature) {
        try {
            ...
            if (response.statusCode() < 200 && response.statusCode() > 299) {
                ...
                System.out.println(document.doc_id);
            }
        } catch (InterruptedException e) {
